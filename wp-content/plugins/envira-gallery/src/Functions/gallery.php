 ...
 */
function envira_load_layout( $template, $data = array() ) {

	$dir = trailingslashit( plugin_dir_path( ENVIRA_FILE ) . 'src/Views/layout' );
...
function envira_santitize_everything( $string, $type = 'title', $for_albums = false ) {
	...
	if ( 'title' === $type ) {
		// $filtered_string = function_exists( 'mb_detect_encoding' ) ? htmlentities( $string, ENT_QUOTES, $encoding ) : htmlentities( $string, ENT_QUOTES, $encoding ); // @codingStandardsIgnoreLine
		if ( $for_albums ) {
	...
	if ( 'opts-caption' === $type || 'item-caption' === $type ) {
...
function envira_santitize_description( $description ) {
	...
	return envira_santitize_everything( $description );
